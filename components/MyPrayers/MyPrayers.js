...
const InProgress = () => {
    ...
    const createPrayer = (title, description, answer, id) => {
        if (!title && !id) return Alert.alert('Error', 'El motivo de oracion necesita un titulo', [
            {
    ...
    return (
        <View style={styles.container}>
            {prayers && <List data={prayers} deletePrayer={deletePrayer} editPrayer={editPrayer} title={'My Prayers'} modalVisible={modalVisible} />}
            <CreateButton setModalVisible={() => handleVisibility()} />
            <CreateModal handleVisibility={handleVisibility} modalVisible={modalVisible} createPrayer={createPrayer} editing={editing} prayerToEdit={prayerToEdit} closeEditing={closeEditing} />
